 ...
 * - Left-click/drag to add terrain, Shift+drag or right-click to remove it
 ...
 * - Real-time movement overlay displayed above tokens during combat
 * - Color-coded visual feedback (green/orange/red based on remaining movement)
 ...
 * - Minimal notifications - only shows terrain penalties
 * 
 * SETTINGS:
      ...
      tilesControl.tools.push(
        {
          ...
          title: "Mark Rough Terrain (Shift+drag to remove)",
...
function resetCombatantMovement(combat) {
  ...
  if (actor) {
    ...
    // Refresh the token overlay to show reset movement
    updateTokenMovementOverlay(token.object);
    
    // Brief notification for token owner only
    if (token.isOwner && !game.user.isGM) {
      ui.notifications.info(`Your turn - ${speed}ft movement`);
    }
  } else {
...
/* -----------------------------
   Movement Overlays
----------------------------- */
function setupMovementOverlays() {
  // Refresh token overlays when tokens are refreshed
  Hooks.on("refreshToken", (token) => {
    updateTokenMovementOverlay(token);
  });
  
  // Refresh all tokens when combat state changes
  Hooks.on("updateCombat", () => {
    canvas.tokens.placeables.forEach(token => updateTokenMovementOverlay(token));
  });
  ...
  Hooks.on("deleteCombat", () => {
    canvas.tokens.placeables.forEach(token => clearTokenMovementOverlay(token));
  });
}

function updateTokenMovementOverlay(token) {
  // Remove existing overlay
  clearTokenMovementOverlay(token);
  
  // Only show overlay during combat
  const inCombat = game.combat?.started && game.combat.combatants.some(c => c.tokenId === token.id);
  if (!inCombat) return;
  ...
  // Only show overlay to GM or token owner
  if (!game.user.isGM && !token.isOwner) return;
  
  // Create overlay graphics
  const overlay = new PIXI.Container();
  overlay.name = "terrainMovementOverlay";
  
  // Calculate color based on remaining movement percentage
  const percent = (movementData.remaining / movementData.total) * 100;
  let color = 0x00ff00; // Green
  if (percent <= 25) color = 0xff0000; // Red
  else if (percent <= 50) color = 0xffaa00; // Orange
  
  // Background
  const bg = new PIXI.Graphics();
  bg.beginFill(0x000000, 0.7);
  bg.drawRoundedRect(-35, -token.h - 25, 70, 20, 5);
  bg.endFill();
  overlay.addChild(bg);
  ...
  border.drawRoundedRect(-35, -token.h - 25, 70, 20, 5);
  overlay.addChild(border);
  
  // Text
  const text = new PIXI.Text(`${movementData.remaining}/${movementData.total}ft`, {
    fontFamily: 'Arial',
    fontSize: 14,
    fontWeight: 'bold',
    fill: color,
  ...
  text.position.set(0, -token.h - 15);
  ...
  icon.position.set(-28, -token.h - 15);
...
function clearTokenMovementOverlay(token) {
  const existing = token.children.find(c => c.name === "terrainMovementOverlay");
  if (existing) {
...
function setupMovementTracking() {
  ...
  Hooks.on("updateToken", (tokenDoc, change, options, userId) => {
    ...
    if (inCombat) {
      ...
      // Refresh the token overlay
      const token = tokenDoc.object;
      if (token) {
        updateTokenMovementOverlay(token);
      }
      
      // Only show brief notification for terrain penalties
      if (penalty > 0) {
        ui.notifications.warn(`${tokenDoc.name}: ${movementCost.difficultSquares} difficult terrain (+${penalty}ft cost)`);
      }
    } else if (!combatOnly && penalty > 0) {
...
function handlePointerDown(event) {
  ...
  // Check if Shift key is held - if so, this is a removal operation
  isRightClick = event.shiftKey;
  isDragging = true;
...
function handlePointerMove(event) {
  ...
  // Update isRightClick based on current Shift key state
  isRightClick = event.shiftKey;
